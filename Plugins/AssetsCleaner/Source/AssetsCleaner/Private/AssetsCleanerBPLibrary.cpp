...
{
	TArray<FAssetData> UnloadedAssets;
	...
	//UE_LOG(LogTemp, Warning, TEXT("Loading %d assets"), UnloadedAssets.Num());
	if (UnloadedAssets.Num() > 0)
	{
		...
		FScopedSlowTask LoadingTask(UnloadedAssets.Num(), FText::FromString("Loading assets"));
		LoadingTask.MakeDialog(true);
		for (FAssetData Asset : UnloadedAssets)
		{
			if (LoadingTask.ShouldCancel())
			{
				Succeeded = false;
				return;
			}
			FString Message = "Loading Asset: " + Asset.AssetName.ToString();
			LoadingTask.EnterProgressFrame(1, FText::FromString(Message));
			Asset.GetAsset();
		}
	}
