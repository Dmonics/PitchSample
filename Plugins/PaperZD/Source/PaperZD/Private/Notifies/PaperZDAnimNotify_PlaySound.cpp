...
void UPaperZDAnimNotify_PlaySound::OnReceiveNotify_Implementation(UPaperZDAnimInstance *OwningInstance /* = nullptr*/)
{
	...
	if (SequenceRenderComponent && Sound)
	{
		...
		if (bFollow)
		{
			UGameplayStatics::SpawnSoundAttached(Sound, SequenceRenderComponent, AttachName, FVector(ForceInit), EAttachLocation::KeepRelativeOffset, false, VolumeMultiplier, PitchMultiplier);
		}
		else
		{
