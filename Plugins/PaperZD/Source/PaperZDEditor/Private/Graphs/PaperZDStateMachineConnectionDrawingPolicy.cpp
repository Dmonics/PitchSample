...
void FPaperZDStateMachineConnectionDrawingPolicy::DetermineWiringStyle(UEdGraphPin* OutputPin, UEdGraphPin* InputPin, /*inout*/ FConnectionParams& Params)
{
	...
	if (InputPin)
	{
		if (UPaperZDAnimGraphNode_Transition* TransNode = Cast<UPaperZDAnimGraphNode_Transition>(InputPin->GetOwningNode()))
		{
			Params.WireColor = SPaperZDAnimGraphNode_Transition::StaticGetTransitionColor(TransNode, HoveredPins.Contains(InputPin));
		}
	}
...
{
	if (UPaperZDAnimGraphNode_Root* EntryNode = Cast<UPaperZDAnimGraphNode_Root>(OutputPin->GetOwningNode()))
	{
		StartWidgetGeometry = PinGeometries->Find(OutputPinWidget);

		UPaperZDAnimGraphNode* State = CastChecked<UPaperZDAnimGraphNode>(InputPin->GetOwningNode());
		int32 StateIndex = NodeWidgetMap.FindChecked(State);
		EndWidgetGeometry = &(ArrangedNodes[StateIndex]);
	}
	else if (UPaperZDAnimGraphNode_Jump* CastNode = Cast<UPaperZDAnimGraphNode_Jump>(OutputPin->GetOwningNode()))
	{
		StartWidgetGeometry = PinGeometries->Find(OutputPinWidget);
	...
	else if (UPaperZDAnimGraphNode_Transition* TransNode = Cast<UPaperZDAnimGraphNode_Transition>(InputPin->GetOwningNode()))
	{
