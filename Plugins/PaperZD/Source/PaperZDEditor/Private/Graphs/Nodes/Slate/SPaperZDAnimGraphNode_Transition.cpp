...
void SPaperZDAnimGraphNode_Transition::Construct(const FArguments& InArgs, UPaperZDAnimGraphNode_Transition* InNode)
{
	this->GraphNode = InNode;
	this->TransitionNode = CastChecked<UPaperZDAnimGraphNode_Transition>(InNode);
	this->UpdateGraphNode();
}

void SPaperZDAnimGraphNode_Transition::Tick(const FGeometry& AllottedGeometry, const double InCurrentTime, const float InDeltaTime)
{
	SGraphNode::Tick(AllottedGeometry, InCurrentTime, InDeltaTime);

	//Check for changes on the node sequence
	if ((bNodeHadValidSequenceConfigured && this->TransitionNode->AnimSequence == nullptr) || (!bNodeHadValidSequenceConfigured && this->TransitionNode->AnimSequence != nullptr))
	{
		bNodeHadValidSequenceConfigured = this->TransitionNode->AnimSequence != nullptr;
		this->UpdateGraphNode();
	}
}

void SPaperZDAnimGraphNode_Transition::GetNodeInfoPopups(FNodeInfoContext* Context, TArray<FGraphInformationPopupInfo>& Popups) const
{
}

void SPaperZDAnimGraphNode_Transition::MoveTo(const FVector2D& NewPosition, FNodeSet& NodeFilter)
{
	// Ignored; position is set by the location of the attached state nodes
}

bool SPaperZDAnimGraphNode_Transition::RequiresSecondPassLayout() const
{
	return true;
}

void SPaperZDAnimGraphNode_Transition::PerformSecondPassLayout(const TMap< UObject*, TSharedRef<SNode> >& NodeToWidgetLookup) const
{
	UPaperZDAnimGraphNode_Transition* TransNode = CastChecked<UPaperZDAnimGraphNode_Transition>(GraphNode);
	...
	UPaperZDAnimGraphNode* PrevState = TransNode->GetFromNode();
	UPaperZDAnimGraphNode* NextState = TransNode->GetToNode();
	if ((PrevState != NULL) && (NextState != NULL))
	{
		...
		if ((pPrevNodeWidget != NULL) && (pNextNodeWidget != NULL))
		{
			...
			{
				UPaperZDAnimGraphNode_Transition *Trans = Cast<UPaperZDAnimGraphNode_Transition>(Pin->GetOwningNode());
...
TSharedRef<SWidget> SPaperZDAnimGraphNode_Transition::GenerateRichTooltip()
{
	UPaperZDAnimGraphNode_Transition* TransNode = CastChecked<UPaperZDAnimGraphNode_Transition>(GraphNode);
...
TSharedPtr<SToolTip> SPaperZDAnimGraphNode_Transition::GetComplexTooltip()
{
...
void SPaperZDAnimGraphNode_Transition::UpdateGraphNode()
{
	InputPins.Empty();
	OutputPins.Empty();
	...
	this->GetOrAddSlot(ENodeZone::Center)
		...
		+ SOverlay::Slot()
			[
				...
				.Image(this, &SPaperZDAnimGraphNode_Transition::GetBodyBrush)
				.ColorAndOpacity(this, &SPaperZDAnimGraphNode_Transition::GetTransitionColor)
			]
		+ SOverlay::Slot()
			.HAlign(HAlign_Center)
			.VAlign(VAlign_Center)
...
const FSlateBrush* SPaperZDAnimGraphNode_Transition::GetBodyBrush() const
{
	return bNodeHadValidSequenceConfigured ? FPaperZDEditorStyle::Get().GetBrush("PaperZDEditor.Graph.TransitionNode.Body_Seq") : FPaperZDEditorStyle::Get().GetBrush("PaperZDEditor.Graph.TransitionNode.Body");
}

FText SPaperZDAnimGraphNode_Transition::GetPreviewCornerText(bool bReverse) const
{
	UPaperZDAnimGraphNode_Transition* TransNode = CastChecked<UPaperZDAnimGraphNode_Transition>(GraphNode);

	UPaperZDAnimGraphNode* PrevState = (bReverse ? TransNode->GetToNode() : TransNode->GetFromNode());
	UPaperZDAnimGraphNode* NextState = (bReverse ? TransNode->GetFromNode() : TransNode->GetToNode());
...
FLinearColor SPaperZDAnimGraphNode_Transition::StaticGetTransitionColor(UPaperZDAnimGraphNode_Transition* TransNode, bool bIsHovered)
{
...
FSlateColor SPaperZDAnimGraphNode_Transition::GetTransitionColor() const
{
	UPaperZDAnimGraphNode_Transition* TransNode = CastChecked<UPaperZDAnimGraphNode_Transition>(GraphNode);
	return StaticGetTransitionColor(TransNode, IsHovered());
}

void SPaperZDAnimGraphNode_Transition::PositionBetweenTwoNodesWithOffset(const FGeometry& StartGeom, const FGeometry& EndGeom, int32 NodeIndex, int32 MaxNodes) const
{
